...
void Game::handleMovementActions() {
  ...
  room * newRoom = &dungeon->rooms
  [player->currentRoom->row + verticalMovement]
  [player->currentRoom->col + horizontalMovement];
...
void Game::handleItemActions() {
  ...
  if (input == 1) {
    ...
    std::cout << "You picked up a " << item.name << ". Your health is now " << player->getHealth() << " and your damage is " << player->getDamage() << ".\n";
